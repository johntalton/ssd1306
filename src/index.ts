...
async function writeCommand(addressedDev: I2CAddressedBus, bufferSource: I2CBufferSource) {
  // await addressedDev.i2cWrite(Uint8Array.from([ 0x00 ]))
	return addressedDev.i2cWrite(bufferSource)
}
...
async function init(addressedDev: I2CAddressedBus) {
  ...
  // await writeCommand(addressedDev, Uint8Array.from([ 0x00, 0xda, 0x01, 0x12 ]))
	...
	const fillValue = 0b0100_0010


	const chunkSize = 60
	for(let i = 0; i <  (128 * 8) / chunkSize; i +=1) {
		await addressedDev.i2cWrite(Uint8Array.from([ 0x40, ...(new Array(chunkSize).fill(fillValue)) ]))
	}

	// await addressedDev.i2cWrite(Uint8Array.from([ 0x40, ...(new Array(32).fill(fillValue))  ]))
	// await addressedDev.i2cWrite(Uint8Array.from([ 0x40, ...(new Array(32).fill(fillValue))  ]))
	// await addressedDev.i2cWrite(Uint8Array.from([ 0x40, ...(new Array(32).fill(fillValue))  ]))
	// await addressedDev.i2cWrite(Uint8Array.from([ 0x40, ...(new Array(32).fill(fillValue))  ]))
	...
	// const height = pages * pixelsPerSegment


	const times = (segments / pattern.length) * pages


	for(let i = 0; i <  times; i +=1) {
		// const n = () => Math.trunc(Math.random() * 255)
		//await addressedDev.i2cWrite(Uint8Array.from([ 0x40, n(), n(), n(), n(), n(), n(), n(), n(), n(), n(), n(), n(), n(), n(), n(), n()  ]))
		await addressedDev.i2cWrite(Uint8Array.from([ 0x40, ...pattern  ]))
	}
